...
<div class="max-w-full mx-auto p-6 sm:px-6 lg:px-8 text-lg">
    <div class="bg-white dark:bg-neutral-900 shadow-lg rounded-xl p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-neutral-100">Buat Rincian Biaya Perjalanan Dinas (RBPD)</h2>
        </div>
        ...
        <form method="POST" action="<?= base_url('dokumen/storeRBPD') ?>">
            ...
            <input type="hidden" name="surat_id" value="<?= esc($surat['id']) ?>">
                ...
                <div class="border border-gray-300 rounded-lg p-4">
                    ...
                    <input type="date" name="tanggal" value="<?= date('Y-m-d') ?>" class="w-full rounded-md border-gray-300 shadow-sm dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-300 p-3" required />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Yang Menerima -->
                <div class="mb-4 border border-gray-300 rounded-lg p-4">
                    <label class="block text-gray-700 dark:text-neutral-300 font-medium mb-2">Yang Menerima</label>
                    <input type="hidden" name="penerima_id" value="<?= esc($penerima['id']) ?>">
                    <input type="text" value="<?= esc($penerima['nama']) ?> (<?= esc($penerima['nip']) ?>)" class="w-full rounded-md border-gray-300 p-3 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-300" readonly />
                </div>
            </div>
            ...
            <div class="mb-6 border border-gray-300 rounded-lg p-4">
                ...
                <?php if (isset($penanda_tangan) && is_array($penanda_tangan)): ?>
                    <select name="id_penanda_tangan" class="w-full rounded-md border-gray-300 p-3" required>
                        <option value="<?= esc($penanda_tangan['id']) ?>"><?= esc($penanda_tangan['nama']) ?> - <?= esc($penanda_tangan['nip']) ?></option>
